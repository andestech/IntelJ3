...
{
	int count = 1;
	char send_data[2], recv_data[6];
	int retval __attribute__((unused));
	send_data[0] = BURNER_NHART;

	retval = SEND(sock, send_data, 1);
	retval = RECV(sock, recv_data, 6);

	count = (((recv_data[2]<< 24) & 0xFF000000) |
	         ((recv_data[3]<< 16) & 0x00FF0000) |
	         ((recv_data[4]<< 8) & 0x0000FF00) |
	         ((recv_data[5]<< 0) & 0x000000FF));

	if (recv_data[0] == BURNER_NHART)
		return count;
	else {
		fprintf(pLogFile, "\n!! nhart: %s, used default 1!!\n", TARGET_ERROR);
		fflush(pLogFile);
		return 1;
	}
}
